...
class PactMacOSExampleTests: XCTestCase {
  ...
  func test_API_returnsCharacter() {
    ...
    starWarsProvider!.uponReceiving("a request for a character").withRequest(method: .GET, path: "/people/1/")
    ...
    starWarsProvider!.run(timeout: 10) { (testCompleted) -> Void in
      self.starWarsClient!.fetchStarWarsCharacter(id: 1) { (response, statusCode) -> Void in
        ...
        XCTAssertTrue(response is SWCharacter)

        let swCharacter = response as! SWCharacter
  ...
  func test_API_returnsError() {
    starWarsProvider!.uponReceiving("an invalid request").withRequest(method: .GET, path: "/people/0/")
      .willRespondWith(status: 400, headers: ["Content-Type": "application/json",
                                              "Authorization": Matcher.somethingLike("Bearer x12345")],
                       body: ["error": Matcher.somethingLike("bar")])
